...
AO's GitHub App integration connects your GitHub repositories to AO Cloud directly — no manual webhook secrets, no personal access tokens. After installation, AO receives GitHub events (push, pull request, issue, and more) via a managed webhook receiver and can act on them through [event triggers](event-triggers.md).
...
2. **Repository selector** — choosing which repositories AO can access.
...
- An AO Cloud account. See [Cloud Deployment](cloud-deployment.md) to get started.
- **Admin** role or above on the AO Cloud organisation. See [Team and Access](cloud-dashboard.md#team-and-access).
...
Navigate to **Settings → Integrations → GitHub** in the AO Cloud dashboard. Click **Install GitHub App**.
...
**Permissions requested by the AO GitHub App:**
...
After you confirm the permissions, GitHub redirects back to the AO Cloud dashboard with a temporary `installation_id`. AO exchanges this for an installation token, stores the mapping, and shows a success state on the Integrations page.
...
The repository selector controls which repositories AO can receive events from and access via the GitHub API. Limiting scope here minimises the blast radius if credentials are compromised.
...
| Mode | Description |
|---|---|
| **All repositories** | AO receives events from every repository in the account now and any added in the future |
| **Selected repositories** | AO receives events only from the repositories you explicitly choose |
...
1. Go to **Settings → Integrations → GitHub** in the AO Cloud dashboard.
...
5. Click **Save**. GitHub sends an `installation_repositories` event to AO; the dashboard updates within a few seconds.
...
After installation, the **Settings → Integrations → GitHub → [installation] → Repositories** tab shows the repositories AO currently has access to:
...
2. GitHub posts a signed webhook payload to AO's receiver endpoint.
3. AO validates the payload signature using the installation's webhook secret (managed internally; never exposed to users).
4. AO parses the event, looks up the matching AO Cloud organisation and project by `installation_id`, and emits an internal `github.*` event.
...
| GitHub event | AO event name | Description |
|---|---|---|
...
Every inbound GitHub webhook is logged in the AO Cloud dashboard at **Settings → Integrations → GitHub → [installation] → Deliveries**.
...
Workflows triggered by GitHub events can write check runs, commit statuses, or PR comments back to GitHub using the `github` MCP tool pack (installed separately via `ao pack install github`).
...
The `github` pack uses the installation token for the repository's installation — no additional credentials are required. Token rotation is handled automatically by the AO Cloud backend.

---

## Revoking the Integration

To remove the GitHub App integration from your AO Cloud organisation:

1. Go to **Settings → Integrations → GitHub**.
2. Click **Revoke** next to the installation.
3. Confirm the action. AO deletes the installation token and stops accepting events for this installation.

This does **not** uninstall the GitHub App from your GitHub account. To fully uninstall, also go to `https://github.com/settings/installations` (or your organisation's equivalent) and click **Uninstall** next to the AO app.
...
- Run `ao trigger list` and confirm the trigger status is `active`, not `paused`.
- Check `ao daemon status` — the daemon must be running to process dispatches.
